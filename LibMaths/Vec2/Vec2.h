	...
	public:
		...
		const bool isUnit() const
		{
			return std::abs(this->length() - Vec2<T>::unitVal) <= std::numeric_limits<T>::epsilon() ||
				std::abs(this->length() - Vec2<T>::unitVal) <= std::numeric_limits<T>::epsilon() * max(abs(this->length()), abs(Vec2<T>::unitVal));
		}
		...
		const bool operator==(const Vec2<T>& vec2) const
		{
			...
			return std::abs(this->length() - vec2.length()) <= std::numeric_limits<T>::epsilon() ||
				std::abs(this->length() - vec2.length()) <= std::numeric_limits<T>::epsilon() * max(abs(this->length()), abs(vec2.length()));
		}

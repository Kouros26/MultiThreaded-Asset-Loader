	...
	public:
		...
		const bool isUnit() const
		{
			return std::abs(this->length() - Vec3<T>::unitVal) <= std::numeric_limits<T>::epsilon() ||
				std::abs(this->length() - Vec3<T>::unitVal) <= std::numeric_limits<T>::epsilon() * max(std::abs(this->length()), std::abs(Vec3<T>::unitVal));
		}
		...
		const bool operator==(const Vec3<T>& vec3) const
		{
			...
			return std::abs(this->length() - vec3.length()) <= std::numeric_limits<T>::epsilon() ||
				std::abs(this->length() - vec3.length()) <= std::numeric_limits<T>::epsilon() * max(abs(this->length()), abs(vec3.length()));
		}

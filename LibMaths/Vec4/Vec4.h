	...
	public:
		...
		const bool isUnit() const
		{
			return std::abs(this->length() - Vec4<T>::unitVal) <= std::numeric_limits<T>::epsilon() ||
				std::abs(this->length() - Vec4<T>::unitVal) <= std::numeric_limits<T>::epsilon() * max(abs(this->length()), abs(Vec4<T>::unitVal));
		}
		...
		const bool operator==(const Vec4<T>& vec4) const
		{
			...
			return std::abs(this->length() - vec4.length()) <= std::numeric_limits<T>::epsilon() ||
				std::abs(this->length() - vec4.length()) <= std::numeric_limits<T>::epsilon() * max(abs(this->length()), abs(vec4.length()));
		}
